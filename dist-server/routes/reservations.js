...
async function resolveLocation(businessUsername, locationId) {
...
router.get("/:businessUsername/:locationId/settings", async (req, res) => {
    try {
        const resolved = await resolveLocation(String(req.params.businessUsername || "").trim(), String(req.params.locationId || "").trim());
        if (!resolved)
