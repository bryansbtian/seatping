...
import { AUDIENCE_GROUPS, MANUAL_AUDIENCE, CUSTOM_GROUP_AUDIENCE, isValidAudienceType, editableVariables, buildMessage, filterRecipients, resolveAudienceGuests, seedSeatPingTemplates, generateUniqueTemplateSlug, normalizeVariableName, normalizeBodyPlaceholders, validateBodyParamPositions, restaurantNameForLocation, wallClockToUtc, advanceRecurrence, formatInstantInTimezone, smsSegments, } from "../lib/campaigns.js";
...
async function serializeCampaign(c, extra) {
    return {
        id: c.id,
        name: c.name,
        channel: c.channel,
...
router.get("/templates", async (req, res) => {
    try {
        ...
        await seedSeatPingTemplates().catch((e) => console.error("[campaigns] seed error:", e?.message || e));
...
router.get("/", async (req, res) => {
    try {
        ...
        const reconciled = await Promise.all(campaigns.map((c) => RECONCILE.has(c.status) ? reconcileCampaign(c) : Promise.resolve(c)));
        ...
        return res.json({
            campaigns: await Promise.all(reconciled.map(async (c) => serializeCampaign(c, {
...
async function validateCampaignInput(businessId, body) {
    const name = String(body?.name || "").trim();
    if (!name)
    ...
    if (audienceType === MANUAL_AUDIENCE) {
        const ids = Array.isArray(body?.audienceConfig?.guestIds)
            ? body.audienceConfig.guestIds.map((x) => String(x)).filter(Boolean)
            : [];
        if (!ids.length)
 ...
 */
async function prepareSend(businessId, campaign) {
    ...
    if (!isTemplateUsable(template)) {
        return {
            error: template.approvalStatus === "PENDING_SEATPING_REVIEW"
...
router.post("/debug/email-test", async (req, res) => {
    ...
    try {
        const emails = Array.isArray(req.body?.emails)
            ? req.body.emails.map((e) => String(e).trim()).filter(Boolean).slice(0, 10)
            : [];
        if (!emails.length) {
            return res.status(400).json({ error: "Provide emails: string[]" });
        }
        const subject = String(req.body?.subject || "SeatPing email delivery test");
        const bodyText = String(req.body?.body || "This is a SeatPing email delivery test. If you received it, delivery is working for this address.");
    ...
    try {
        ...
        if (sendMode === "RECURRING") {
            ...
            while (next.getTime() <= Date.now()) {
                next = advanceRecurrence(next, freq, timezone);
            }
            ...
            const claim = await prisma.campaign.updateMany({
                ...
                data: {
                    ...
                    maxSendsPerGuestWindowDays: Number.isFinite(windowDays) && windowDays > 0 ? Math.floor(windowDays) : 30,
                    nextRunAt: next,
                    isPaused: false,
                },
            });
...
router.post("/:id/resume", async (req, res) => {
    try {
        ...
        const freq = (campaign.recurrenceFrequency || "WEEKLY");
...
router.post("/preview-message", async (req, res) => {
    try {
        ...
        const message = buildMessage(template, filled, {
            businessName: restName,
            businessEmail: business.email,
