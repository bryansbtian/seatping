...
router.post("/business/:username/queue", async (req, res) => {
    try {
        ...
        if (!username)
            return res.status(400).json({ error: "username is required" });
        const { address, firstName, lastName, numGuests, phoneNumber, countryCode, waitingPreference, smsConsent, } = req.body || {};
        if (!address ||
            !firstName ||
            !lastName ||
            !numGuests ||
        ...
        if (waitingPreference === "wait_anywhere") {
            ...
            if (locationSmsCredits <= 0) {
                return res.status(400).json({
                    error: "This location has insufficient SMS credits for Wait Anywhere service",
                });
            }
        }
...
router.get("/business/:username/queue/:customerId/status", async (req, res) => {
    try {
        ...
        for (const location of locations) {
            const queue = location.queue || [];
            const customerIndex = queue.findIndex((c) => c.firstName + c.lastName + c.joinedAt === customerId);
            if (customerIndex !== -1) {
                customerFound = true;
        ...
        for (const location of locations) {
            // Check admitted customers
            const admittedCustomers = location.admittedCustomers || [];
            const admittedCustomer = admittedCustomers.find((c) => c.firstName + c.lastName + c.joinedAt === customerId);
            if (admittedCustomer) {
                return res.json({
                    admitted: true,
                    removed: false,
            ...
            // Check removed customers
            const removedCustomers = location.removedCustomers || [];
            const removedCustomer = removedCustomers.find((c) => c.firstName + c.lastName + c.joinedAt === customerId);
            if (removedCustomer) {
                return res.json({
                    admitted: false,
                    removed: true,
 ...
 */
router.post("/business/:username/queue/:customerId/admit", requireAuth, async (req, res) => {
    try {
        const userId = req.auth.sub;
        const username = String(req.params.username || "").trim();
        const customerId = String(req.params.customerId || "").trim();
        ...
        for (let i = 0; i < locations.length; i++) {
            ...
            if (customerIndex !== -1) {
                ...
                if (locationCustomerCredits <= 0) {
                    return res.status(400).json({
                        error: "This location has insufficient customer credits. Please upgrade your plan.",
                    });
                }
        ...
        const location = locations[locationIndex];
        location.customerCredits = Math.max(0, (location.customerCredits || 0) - customerCreditsToDeduct);
        // Deduct SMS credit if customer opted for it
        if (admittedCustomer.waitingPreference === "wait_anywhere") {
            smsCreditsToDeduct = 1;
            location.smsCredits = Math.max(0, (location.smsCredits || 0) - smsCreditsToDeduct);
        }
 ...
 */
router.post("/business/:username/admitted/:customerId/confirm-arrival", requireAuth, async (req, res) => {
    try {
        const userId = req.auth.sub;
        const username = String(req.params.username || "").trim();
        const customerId = String(req.params.customerId || "").trim();
        ...
        for (let i = 0; i < locations.length; i++) {
            ...
            const customerIndex = admittedCustomers.findIndex((c) => c.firstName + c.lastName + c.joinedAt === customerId);
            if (customerIndex !== -1) {
 ...
 */
router.post("/business/:username/admitted/:customerId/mark-no-show", requireAuth, async (req, res) => {
    try {
        const userId = req.auth.sub;
        const username = String(req.params.username || "").trim();
        const customerId = String(req.params.customerId || "").trim();
 ...
 */
router.delete("/business/:username/queue/:customerId", requireAuth, async (req, res) => {
    try {
        const userId = req.auth.sub;
        const username = String(req.params.username || "").trim();
        const customerId = String(req.params.customerId || "").trim();
...
router.put("/me", requireAuth, async (req, res) => {
    try {
        ...
        const processedLocations = locations.map((location) => {
            ...
            const existingLocation = currentLocations.find((loc) => loc.address === location.address);
            if (existingLocation) {
                ...
                return createLocationWithTrialEnforcement(currentUser, location.address);
            }
        });
        const updated = await prisma.user.update({
            where: { id: userId },
