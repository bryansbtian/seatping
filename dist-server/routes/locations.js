...
function pickCuisineSuggest(rp) {
...
function fieldScore(value, needle) {
    if (!value)
        return 0;
...
router.get("/search-suggestions", async (req, res) => {
    try {
        ...
        const scored = locations
            ...
            const labelScore = Math.max(fieldScore(loc.displayName, q), fieldScore(rp.shortAddress, q), fieldScore(loc.name, q));
            const areaCityScore = Math.max(fieldScore(loc.area, q), fieldScore(loc.city, q), fieldScore(details.area, q), fieldScore(details.city, q));
            const cuisineArr = Array.isArray(rp.cuisineTypes) ? rp.cuisineTypes : [];
            const cuisineScore = cuisineArr.reduce((m, c) => Math.max(m, fieldScore(c, q)), 0);
            const addressScore = Math.max(fieldScore(loc.address, q), fieldScore(details.address, q));
            const textScore = Math.max(fieldScore(rp.tagline, q), fieldScore(rp.description, q), fieldScore(biz?.username, q));
            const score = nameScore * SUGGEST_WEIGHTS.name +
 ...
 */
async function loadOwnedLocation(req, res, next) {
    try {
...
async function refreshedUser(req) {
    return assembleBusinessMe(req.auth.sub);
}
// ===========================================================================
 ...
 */
router.post("/:locationId/banner/upload", loadOwnedLocation, async (req, res) => {
    try {
        try {
 ...
 */
router.post("/:locationId/banner/sign", loadOwnedLocation, async (_req, res) => {
    try {
        const location = res.locals.location;
 ...
 */
router.post("/:locationId/banner/commit", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
 ...
 */
router.delete("/:locationId/banner", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        await prisma.location.update({
            where: { id: location.id },
 ...
 */
router.post("/:locationId/photos/upload", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        ...
        try {
            await runMulter(req, res, upload.array("files", MAX_PHOTOS_PER_LOCATION));
        }
        catch (err) {
            return res.status(400).json({ error: uploadErrorMessage(err) });
        }
        ...
        const uploaded = await Promise.all(files.map((f) => uploadImageBuffer(f.buffer, location.id, "photo")));
        const created = [];
 ...
 */
router.post("/:locationId/photos/sign", loadOwnedLocation, async (_req, res) => {
    try {
        const location = res.locals.location;
        const existingCount = await prisma.photo.count({
            where: { locationId: location.id },
 ...
 */
router.post("/:locationId/photos/commit", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        const { url, publicId } = (req.body || {});
        if (typeof url !== "string" ||
 ...
 */
router.patch("/:locationId/photos/:photoId", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        ...
        if (!photo)
            return res.status(404).json({ error: "Photo not found" });
        const trimmed = typeof altText === "string" && altText.trim() ? altText.trim() : null;
 ...
 */
router.delete("/:locationId/photos/:photoId", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        const photoId = String(req.params.photoId || "").trim();
        if (!OBJECT_ID_RE.test(photoId)) {
 ...
 */
router.get("/:locationId/reviews", loadOwnedLocation, async (_req, res) => {
    try {
        const location = res.locals.location;
 ...
 */
router.patch("/:locationId/reviews/:reviewId/reply", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
 ...
 */
router.delete("/:locationId/reviews/:reviewId/reply", loadOwnedLocation, async (req, res) => {
    try {
        const location = res.locals.location;
        const reviewId = String(req.params.reviewId || "").trim();
        if (!OBJECT_ID_RE.test(reviewId)) {
