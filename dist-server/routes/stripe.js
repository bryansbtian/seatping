...
const PRICE_IDS = {
    ...
    "Starter Yearly": process.env.STRIPE_PRICE_STARTER_YEARLY ?? "price_1SKp2dALbq0s7SXlGbKp3Ivg",
    "Professional Monthly": process.env.STRIPE_PRICE_PRO_MONTHLY ?? "price_1SKoytALbq0s7SXlUt8FlC7z",
    "Professional Yearly": process.env.STRIPE_PRICE_PRO_YEARLY ?? "price_1SKox3ALbq0s7SXlBXVKT9tM",
};
const PLAN_RULES = {
    Starter: {
        plan: "Starter",
...
const normalizePlanName = (name) => {
    ...
    if (n.startsWith("starter"))
        return "Starter";
    if (n.startsWith("professional") || n === "pro")
        return "Professional";
    return null;
};
async function applyPlanToUser(userId, plan, setStartTime) {
    ...
    try {
        ...
        try {
            ...
            const locations = userWithLocations?.locations || [];
            if (locations.length > 0) {
                ...
                console.log(`[stripe] ✅ Updated ${locations.length} existing locations with base credits (SMS=${userWithLocations?.baseSMSCredits}, Customers=${userWithLocations?.baseCustomerCredits})`);
            }
            else {
    ...
    try {
        switch (event.type) {
            case "checkout.session.completed": {
                ...
                let plan = normalizePlanName(session.metadata?.plan) ||
                    null;
