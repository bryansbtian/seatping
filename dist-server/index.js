...
if (process.env.VERCEL !== '1') {
    ...
    runDailyCreditRefillSweep().catch((err) => console.error("[CREDIT-SWEEP] initial run failed:", err));
    setInterval(() => {
        runDailyCreditRefillSweep().catch((err) => console.error("[CREDIT-SWEEP] scheduled run failed:", err));
    }, DAILY_MS);
    ...
    runReservationReminderSweep().catch((err) => console.error("[RESERVATION-REMINDER] initial run failed:", err));
    setInterval(() => {
        runReservationReminderSweep().catch((err) => console.error("[RESERVATION-REMINDER] scheduled run failed:", err));
    }, REMINDER_MS);
    ...
    setInterval(() => {
        runDueCampaignsSweep().catch((err) => console.error("[CAMPAIGN-SWEEP] scheduled run failed:", err));
    }, CAMPAIGN_SWEEP_MS);
}
export default app;
