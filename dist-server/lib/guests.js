...
import { getLocationTimezone, getNowWallClockInTimezone, DEFAULT_LOCATION_TIMEZONE, } from "./operatingHours.js";
...
export function normalizePhone(phone, countryCode) {
    const raw = `${typeof countryCode === "string" ? countryCode : ""}${typeof phone === "string" ? phone : ""}`;
...
export function computeStats(queueRows, reservationRows, timezone = DEFAULT_LOCATION_TIMEZONE) {
    const now = Date.now();
    ...
    for (const r of reservationRows) {
        ...
        const alreadyHappened = !!r.reservationDateTime &&
...
export function buildSummary(stats, timeZone) {
    ...
    if (stats.upcomingReservationCount > 0) {
        parts.push(`Has ${stats.upcomingReservationCount} upcoming reservation${stats.upcomingReservationCount === 1 ? "" : "s"}.`);
    }
    if (stats.noShowCount > 0) {
        parts.push(`Has ${stats.noShowCount} no-show${stats.noShowCount === 1 ? "" : "s"}.`);
    }
    return parts.join(" ");
}
/** True when the guest's visit count makes them a repeat/"Returning" guest. */
export function isReturning(totalVisits) {
 ...
 */
export async function recomputeGuestStats(guestId) {
 ...
 */
export async function upsertGuestForVisit(input) {
...
export async function syncGuestFromQueueEntry(entry, opts = {}) {
    try {
...
export async function syncGuestFromReservation(row, opts = {}) {
    try {
        await upsertGuestForVisit({
 ...
 */
export async function touchGuestByQueueEntryId(entryId) {
    try {
...
export async function touchGuestByReservationId(reservationId) {
    try {
        const guest = await prisma.guestProfile.findFirst({
 ...
 */
export async function loadGuestBadgeMap(businessId) {
...
export function badgeForContact(map, contact) {
