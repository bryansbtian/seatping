...
export function logRateLimitStatus() {
    ...
    if (isProd) {
        ...
        console.log("[rate-limit] backend=memory (in-process fallback) — fine for local dev. " +
...
function memoryLimit(fullKey, windowMs, max) {
    const now = Date.now();
...
function memoryPeek(fullKey, windowMs, max) {
    const now = Date.now();
    const existing = memoryBuckets.get(fullKey);
    if (!existing || now >= existing.resetAt)
        return true; // fresh window
...
function getRedisLimiter(windowMs, max) {
 ...
 */
async function checkOne(fullKey, windowMs, max) {
    if (redis) {
        try {
...
function send429(res, retryAfterSec, message) {
    if (retryAfterSec > 0)
 ...
 *   ])) return;
 */
export async function limitGuard(req, res, rules, message) {
    const active = rules.filter((r) => r.key != null && String(r.key).trim() !== "");
    if (active.length === 0)
        return false;
    const results = await Promise.all(active.map((r) => checkOne(`${r.name}:${String(r.key).trim()}`, r.windowMs, r.max)));
 ...
 */
export async function consumeQuota(name, key, windowMs, max) {
 ...
 */
export async function peekQuota(name, key, windowMs, max) {
    if (key == null || String(key).trim() === "")
        return true;
