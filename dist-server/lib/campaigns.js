 ...
 */
export function normalizeVariableName(raw) {
    return String(raw || "")
 ...
 */
export function normalizeBodyPlaceholders(body) {
    if (!body)
        return "";
 ...
 */
export function validateBodyParamPositions(body) {
    if (!body)
        return null;
    const firstOpen = body.indexOf("{{");
    if (firstOpen === -1)
        return null; // no variables at all
    const lastClose = body.lastIndexOf("}}");
    const hasWord = (s) => /[a-z0-9]/i.test(s);
...
export function isValidAudienceType(type) {
...
export function renderString(template, values) {
    if (!template)
        return "";
...
export function editableVariables(template) {
 ...
 */
function buildValueMap(filled, ctx) {
    return {
        ...filled,
 ...
 */
export function buildMessage(template, filled, ctx, channel) {
...
function zonedDayRange(timeZone, dayOffset) {
    const now = new Date();
...
function tzOffsetMs(instant, timeZone) {
    ...
    const asUtc = Date.UTC(Number(map.year), Number(map.month) - 1, Number(map.day), map.hour === "24" ? 0 : Number(map.hour), Number(map.minute), Number(map.second));
 ...
 */
export function restaurantNameForLocation(loc, fallback) {
    const profile = loc?.restaurantProfile && typeof loc.restaurantProfile === "object"
 ...
 */
export function wallClockToUtc(local, timeZone) {
 ...
 */
export function advanceRecurrence(from, frequency, timeZone) {
    const parts = new Intl.DateTimeFormat("en-US", {
        timeZone,
        year: "numeric",
        month: "2-digit",
 ...
 */
export async function resolveAudienceGuests(q) {
    const base = { businessId: q.businessId, locationId: q.locationId };
    ...
    switch (q.audienceType) {
        ...
        case "with_tag": {
            ...
            if (!tagsRaw.length)
                return [];
            where.id = { in: tagsRaw.map((t) => t._id.$oid) };
            break;
        }
        ...
        case "custom_group": {
            ...
            if (filters.tags && Array.isArray(filters.tags) && filters.tags.length > 0) {
                ...
                const idSets = await Promise.all(filters.tags.map(async (tag) => {
                    ...
                    return tagsRaw.map((t) => t._id.$oid);
                }));
...
function optedOut(guest, channel) {
...
export function filterRecipients(guests, channel) {
    ...
    const excludedCount = exclusions.noEmail + exclusions.noPhone + exclusions.optedOut + exclusions.invalid;
...
export function isValidMetaTemplateName(value) {
 ...
 */
export async function generateUniqueTemplateSlug(name, opts = {}) {
    const base = slugifyTemplateName(name);
    const scope = opts.businessId
...
export const SEATPING_TEMPLATE_SEEDS = [
    {
        name: "We Miss You",
 ...
 */
export async function seedSeatPingTemplates() {
    if (seedPromise)
        return seedPromise;
    seedPromise = (async () => {
