...
export function bucketOf(reservationDateTime) {
 ...
 */
export async function tryReserveCapacity(locationId, dateKey, hour, guestCount, cap) {
    if (guestCount <= 0)
        return true;
    if (guestCount > cap)
        return false;
    // Ensure the counter exists (no-op if it already does).
    await withWriteRetry(() => prisma.slotCounter.upsert({
        where: { locationId_dateKey_hour: { locationId, dateKey, hour } },
        create: { locationId, dateKey, hour, reservedGuests: 0 },
        update: {},
    }));
    const result = await withWriteRetry(() => prisma.slotCounter.updateMany({
        where: {
            locationId,
            dateKey,
            hour,
 ...
 */
export async function addCapacity(locationId, dateKey, hour, guestCount) {
    if (guestCount <= 0)
        return;
    await withWriteRetry(() => prisma.slotCounter.upsert({
        where: { locationId_dateKey_hour: { locationId, dateKey, hour } },
...
export async function releaseCapacity(locationId, dateKey, hour, guestCount) {
    if (guestCount <= 0)
        return;
    // Guard `>= guestCount` so a double-release can't drive the counter negative.
    await withWriteRetry(() => prisma.slotCounter.updateMany({
 ...
 */
export async function recountSlots(locationId) {
