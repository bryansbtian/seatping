...
const CHANNEL_META: Record<Channel, { label: string; icon: IconSvgElement }> = {
  EMAIL: { label: "Email", icon: Mail01Icon },
  WHATSAPP: { label: "WhatsApp", icon: MessageCircleIcon },
  SMS: { label: "SMS", icon: Call02Icon },
};
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <FieldTrigger icon={Calendar01Icon} className="bg-slate-50">
          {triggerLabel}
        </FieldTrigger>
      </PopoverTrigger>
      ...
      <CardHeader className="border-b border-slate-200 p-4 md:p-6 flex-row items-center justify-between space-y-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRefresh} disabled={loading}>
            <HugeiconsIcon icon={Refresh01Icon} className={`w-4 h-4 ${refreshIconClass}`} />
          </Button>
          <Button
            size="sm"
            onClick={onNew}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {audiences.map((a) => (
          <div key={a.key} className="border border-slate-200 rounded-xl p-4 bg-slate-50/50">
            <div className="flex items-center gap-2">
              <HugeiconsIcon icon={UsersRoundIcon} className="w-4 h-4 text-indigo-500" />
        ...
        {savedAudiences.map((a) => (
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  <HugeiconsIcon icon={UsersRoundIcon} className="w-4 h-4 text-emerald-500" />
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex items-center gap-3 flex-wrap">
          <span className="flex items-center gap-1">
            <HugeiconsIcon icon={channelIcon} className="w-3.5 h-3.5" /> SeatPing on behalf of your
            restaurant
          </span>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-1 items-center">
            <div className="flex flex-1 gap-2 w-full">
              ...
              <Button
                ...
                className="flex-1"
              >
                <HugeiconsIcon icon={SentIcon} className="w-4 h-4 mr-1.5" />
                {launchButtonLabel}
              </Button>
            </div>
            {existing && (
