...
// ---------------------------------------------------------------------------
// Campaign detail dialog (history view + delivery logs)
// ---------------------------------------------------------------------------
          ...
          <>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {campaign.name}
                <CampaignStatusBadge
                  status={detail?.campaign?.status ?? campaign.status}
                />
              </DialogTitle>
              <DialogDescription>
                {campaign.templateName} · {campaign.locationLabel ?? ""}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <Stat
                label="Recipients"
                value={
                  detail?.campaign?.recipientCount ?? campaign.recipientCount
                }
              />
              <Stat
                label="Sent"
                value={detail?.campaign?.sentCount ?? campaign.sentCount}
                tone="green"
              />
              <Stat
                label="Failed"
                value={detail?.campaign?.failedCount ?? campaign.failedCount}
                tone="red"
              />
              <Stat
                label="Excluded"
                value={
                  detail?.campaign?.excludedCount ?? campaign.excludedCount
                }
              />
            </div>
            ...
            <div className="flex items-center gap-2 text-sm text-slate-600 flex-wrap">
              ...
              <span>Audience: {campaign.audienceLabel}</span>
              <span className="text-slate-400">
                · {timingSummary(campaign)}
              </span>
            </div>
                  ...
                  {detail.runs.map((r: any) => (
                    ...
                    >
                      <span className="text-slate-600">
                        {r.runType} · {fmtDate(r.createdAt)}
                      </span>
                      <span className="text-slate-500">
                        {r.sentCount} Sent · {r.failedCount} Failed ({r.status})
                      </span>
                    </div>
                  ))}
                </div>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-center">
      <div
        className={`text-lg font-semibold tabular-nums ${tone === "green" ? "text-emerald-600" : tone === "red" ? "text-red-600" : "text-slate-800"}`}
      >
        {value}
      </div>
