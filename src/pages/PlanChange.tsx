        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="shadow-xl rounded-xl border-0">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl sm:text-3xl lg:text-4xl font-bold">Change Your Plan</CardTitle>
              {selectedPlan && (
              ...
              <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                {/* Plan Selection Cards */}
                  <div className="space-y-4 sm:space-y-6">
                    <h3 className="text-base sm:text-lg font-semibold text-center text-gray-800">Select New Plan</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      {["Starter", "Professional"].map((plan) => (
                        <Card
                          ...
                          onClick={() => !isCurrentPlan(plan) && handlePlanChange(plan)}
                        >
                          <CardHeader className="text-center px-4 sm:px-6">
                            <CardTitle className={`text-base sm:text-lg ${isCurrentPlan(plan) ? "text-gray-500" : ""}`}>
                              {plan}
                              {isCurrentPlan(plan) && (
                            ...
                            <CardDescription className={`text-sm sm:text-base ${isCurrentPlan(plan) ? "text-gray-400" : ""}`}>
                              {getPlanPrice(plan)}{" "}
                          ...
                          <CardContent className="px-4 sm:px-6">
                            <ul className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
                              {getPlanFeatures(plan).map((feature, index) => (
                                <li key={index} className={isCurrentPlan(plan) ? "text-gray-400" : ""}>
                                  {feature}
                                </li>
                              ))}
                            </ul>
                            <div className="mt-3 sm:mt-4">
                              ...
                              variant={isCurrentPlan(plan) ? "outline" : selectedPlan === plan ? "default" : "outline"}
                            >
                              {isCurrentPlan(plan) ? "Current Plan" : selectedPlan === plan ? "Selected" : "Select"}
                            </Button>
                            </div>
                          </CardContent>
                        </Card>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to change your plan to <strong>{selectedPlan}</strong>? 
                This will update your billing and credits immediately.
              </p>
