...
vi.mock("../../server/lib/prisma.js", () => {
  return {
    prisma: {
      reservation: { findMany: reservationFindMany, update: reservationUpdate },
...
describe("reminder sweep delivery", () => {
  it("enqueues a reminder and stamps the reservation", async () => {
    ...
    expect(reservationUpdate.mock.calls[0][0].data.reminderEmailSentAt).toBeInstanceOf(
      Date,
    );
  });
...
describe("reminder sweep failures", () => {
  it("keeps sweeping when one reminder cannot be enqueued", async () => {
    ...
    expect(reservationUpdate).toHaveBeenCalledTimes(1);
  });
  ...
  it("keeps sweeping when the stamp write fails", async () => {
    reservationFindMany.mockResolvedValue([reservation()]);
